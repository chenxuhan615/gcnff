...
def direct_fix_train(config_file):
    config = load_file(config_file)
    print("\n\t------------------Step--Three'---------------------\n")
    print("\tTraining an 'initial model' with fixed parameters in Conv layers without divide_set.\n")
    try:
    ...
    try:
        randomSeed=config['direct_fix_train']['RandomSeed']
    except:
        randomSeed=0
        print("\tYou did not set the value of ->['direct_fix_train']['RandomSeed']<-, and it has been set to 0 by default!\n")
    try:
        file_path = config['direct_fix_train']['graphfile_path']
    except:
        file_path = "./graphdata"
        print("\tYou did not set the value of ->['direct_fix_train']['graphfile_path']<-, and it has been set to './graphdata/' by default!\n")
    try:
        date_for_initmodel=config['direct_fix_train']['initmodel_data']
    except:
        date_for_initmodel=0.25
        print("\tYou did not set the value of ->['direct_fix_train']['initmodel_data']<-, and it has been set to 0.25 by default!\n")
    try:
        traindate_for_initmodel=config['direct_fix_train']['initmodel_traindata']
    except:
        traindate_for_initmodel=0.75
        print("\tYou did not set the value of ->['direct_fix_train']['initmodel_traindata']<-, and it has been set to 0.75 by default!\n")
    ...
    for i in range(max_epoch):
        ...
        for file in file_names:
            ...
            init_train_size =len(graph_list[:N_init_training])
            init_valid_size =len(graph_list[N_init_training:N_init_data])
            #print("\tRead the ->",file,"<- success! Total configuration:",len(graph_list))
            if(init_train_size==0)or(init_valid_size==0):
                print("\tthe ->",file,"<- has configurations:",len(graph_list),". Too small! We ignored it!")
                continue
            if(is_pin_memory=="True"):
                init_train_dataloader = DataLoader(graph_list[:N_init_training], batch_size=batch_num, pin_memory=True)
                init_valid_dataloader = DataLoader(graph_list[N_init_training:N_init_data], batch_size=batch_num, pin_memory=True)
            else:
                init_train_dataloader = DataLoader(graph_list[:N_init_training], batch_size=batch_num, pin_memory=False)
                init_valid_dataloader = DataLoader(graph_list[N_init_training:N_init_data], batch_size=batch_num, pin_memory=False)
...
def direct_free_train(config_file):
    config = load_file(config_file)
    ...
    try:
        init_model=config['direct_free_train']['begin_model']
    except:
        print("\tYou did not set the value of ->['direct_free_train']['begin_model']<- !\n")
    try:
        randomSeed=config['direct_free_train']['RandomSeed']
    except:
        randomSeed=0
        print("\tYou did not set the value of ->['direct_free_train']['RandomSeed']<-, and it has been set to 0 by default!\n")
    try:
        file_path = config['direct_free_train']['graphfile_path']
    except:
        file_path = "./graphdata"
        print("\tYou did not set the value of ->['direct_free_train']['graphfile_path']<-, and it has been set to './graphdata/' by default!\n")
    try:
        traindate_for_finalmodel=config['direct_free_train']['traindata']
    except:
        traindate_for_finalmodel=0.7
        print("\tYou did not set the value of ->['direct_free_train']['traindata']<-, and it has been set to 0.7 by default!\n")
    if (Flag_AutomaticWeightedLoss=="True"):
        awl = AutomaticWeightedLoss(3)
        try:
